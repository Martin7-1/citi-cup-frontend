<template>
  <div id="container">
    ...
    <div>
      <head-title :isNotLogin=true style="position: absolute; top: 0; z-index: 11" :is-register="isRegister"></head-title>
    </div>
    ...
    <el-card class="card2" shadow="hover">
      ...
      <div class="items">
        ...
        <div style="width: 80%">
          <button style="cursor: pointer;" class="btn login" @click.prevent="register">
            注册
          </button>
        </div>
        <div style="width: 80%;">
...
export default {
  data(){
    return{
      isRegister: this.$store.state.headerViews === 'Register',
    }
  },
  beforeCreate() {
    this.$store.commit('changeViews', 'Register')
  },
}
</script>
...
// phone, password and verify code
const phone = ref("");
const password = ref("");
const username = ref("");
const email = ref("");
...
const nameAndPasswd = computed(() => {
  return phone.value !== "" && password.value !== "" && username.value !== "" && email.value !== "";
});
</script>
